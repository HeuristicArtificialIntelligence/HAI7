    ...
    <footer className="border-t border-[var(--color-border)]">
      <div className="container py-8">
        <div className="flex flex-col lg:flex-row lg:justify-between items-center gap-8">
          <a href="/"><div className="font-extrabold text-2xl">HAI7.ai</div></a>
            ...
            {socialLinks.map((link)=>(
              <a href={link.href} key={link.name}>
                <div className="size-10 rounded-full bg-gray-900 inline-flex items-center justify-center">
                  <FontAwesomeIcon icon={link.icon} className="size-4 text-violet-300" />
                </div>
              </a>
            ))}
          </div>
          <p className="text-gray-500 text-sm">&copy; HAI7, All rights reserved.</p>
        </div>
      </div>
    </footer>
  )};
